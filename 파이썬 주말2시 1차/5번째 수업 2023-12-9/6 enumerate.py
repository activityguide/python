...
# range() : 전달된 인수값에 따라 시퀸스 자료형의 데이터를 작성
# 1) range(stop) / 0부터 stop 사이의 모든 정수가 생성, 새성되는 정수를 n이라고 하면 n의 범위는 0<= n < stop
# 2) range(start, stop) / start부터 stop사이의 모든 정수가 생성, 생성되는 정수를 n이라고 하면 n의 범위는 start <= n < stop
# 3) range(start, stop, step) start부터 stop 사이의 정수 중에서 step 만큼의 차이를 가지고 있다.
#
print(list(range(10)))
...
#
# # len 함수에 전달된 객체의 길이 (항목 수) 전환, 묶음수가 몇개 인지
print()
li = ['a', 'b', 'c', 'd']
print(len(li))
...
# range() 함수의 리스트의 길이를 구하는 len() 함수를 함꼐 사용하면 리스트의 인덱스 생성 가능

seasons = ['봄', '여름', '가을', '겨울']
seasons_eng = ['spring', 'summer', 'fall', 'winter']
for idx in range(len(seasons)):
    print(idx, end=" ")
    print(f'{seasons[idx]}/ {seasons_eng[idx]}', end="          ")

# sorted() 전달된 반복가능객체의 오름차순 정렬 결과를 변환
...
# 튜플은 인패킹이 가능하므로 다음과 같은 모습으로 구성 가능
